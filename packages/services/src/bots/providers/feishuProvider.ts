...
import type {
  BotProviderAdapter,
  BotProviderCreateGroupChatParams,
...
// 改造版：POST /open-apis/im/v1/chats 建群响应，只需要 chat_id。
interface FeishuCreateGroupChatResponse {
  code?: number;
  msg?: string;
  error?: {
    log_id?: string;
...
/**
 * 改造版：创建飞书群聊（拉群建项目）。
 * - POST /open-apis/im/v1/chats?uuid=<uuid>，body 携带群名与初始成员 open_id；
 * - bot 作为创建者自动入群，返回 data.chat_id 供路由绑定；
 * - uuid 作为幂等键，超时重试不会建出重复群。
 */
async function createFeishuGroupChat(
  bot: BotConfig,
  token: string,
  params: BotProviderCreateGroupChatParams,
): Promise<{ chatId: string }> {
  const query = params.uuid ? `?uuid=${encodeURIComponent(params.uuid)}` : "";
  const response = await fetchBotProviderJson<FeishuCreateGroupChatResponse>(
    `${getFeishuBaseUrl(bot)}/open-apis/im/v1/chats${query}`,
    {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
      ...
      body: JSON.stringify({
        ...
        ...(params.memberOpenIds && params.memberOpenIds.length > 0
          ? { user_id_list: params.memberOpenIds }
          : {}),
      }),
    },
  );
  const payload = response.payload ?? {};
  // HTTP 成功也可能携带业务拒绝，必须保留错误码和请求 ID（与发消息同策略）。
  if (!response.ok || payload.code !== 0) {
    throw createFeishuMessageError(
      "create group chat",
      response.status,
  ...
  if (!payload.data?.chat_id) {
    throw new Error("Feishu create group chat failed: missing chat_id in response.");
  }
  return { chatId: payload.data.chat_id };
}
...
export function createFeishuBotProvider(deps: FeishuProviderDeps): BotProviderAdapter {
  ...
  return {
    ...
    async createGroupChat(bot, params) {
      const token = await readTenantAccessToken(bot, deps);
      if (!token) {
        throw new Error("Feishu app credentials are missing.");
      }
      return createFeishuGroupChat(bot, token, params);
    },
